...
import type { RawArgInputs } from '../builders/CommandBuilder/chain2.js'
...
export const parse = (
  ...
  parameterInputs: Record<
    string,
    ParameterBasicInput | ParameterExclusiveInput
  >,
  ...
  const argInputsPrompter =
    argInputs?.tty ??
    (process.stdout.isTTY ? Prompter.createProcessPrompter() : null)
  ...
  const parseProgressPostPromptAnnotation = {
    ...
    basicParameters: Object.fromEntries(
      Object.entries(openingArgsResult.basicParameters).map(
  ...
  if (argInputsPrompter) {
    ...
    for (const spec of basicSpecs) {
      ...
      const eventPatterns = Array.isArray(eventPatterns_)
        ? eventPatterns_
        : [eventPatterns_]
      for (const pattern of eventPatterns) {
        if (match(event, pattern)) {
          parseProgressPostPromptAnnotation.basicParameters[
            spec.name.canonical
          ]!.prompt.enabled = true
          continue
        }
  ...
  const argumentErrors = [
    ...Object.entries(parseProgressPostPromptAnnotation.basicParameters)
      .map(([_, v]): null | ParseResultBasicError => {
        return v.prompt.enabled === false &&
          v.openingParseResult._tag === `error`
          ? v.openingParseResult
          : null
      })
      .filter((_): _ is ParseResultBasicError => _ !== null),
    ...Object.entries(
      parseProgressPostPromptAnnotation.mutuallyExclusiveParameters,
    )
  ...
  if (
    parseProgressPostPromptAnnotation.globalErrors.length > 0 ||
    ...
    if (settings.helpOnError) {
      const message =
        ...
        argumentErrors
          .map((_) => _.errors.map((_) => _.message).join(`\nX `))
          .join(`\nX `) +
        `\n\nHere are the docs for this command:\n`
      settings.onOutput(message + `\n`)
      settings.onOutput(
        Help.render(parametersResult.parameters, settings) + `\n`,
      )
    }
  ...
  const hasPrompt =
    Object.values(parseProgressPostPromptAnnotation.basicParameters).some(
      (_) => _.prompt.enabled,
    ) && argInputsPrompter
    ...
    const args = {
      ...
      ...Object.fromEntries(
        Object.values(parseProgressPostPrompts.mutuallyExclusiveParameters)
          .filter(
            (_): _ is ParseResultExclusiveGroupSupplied =>
              _._tag === `supplied`,
          )
  ...
  return hasPrompt
    ? Effect.runPromise(
        prompt(parseProgressPostPromptAnnotation, argInputsPrompter),
      ).then(tailProcess)
