...
export namespace State {
  ...
  // prettier-ignore
  export type ValidateNameExpression<NameExpression extends string> = 
    FlagName.Errors.$Is<FlagName.Parse<NameExpression, { usedNames: never; reservedNames: ReservedParameterNames }>> extends true
        ? FlagName.Parse<NameExpression, { usedNames: never; reservedNames: ReservedParameterNames }>
        : NameExpression
  ...
  export type MinimumParameterBuilderState = Omit<Parameter.BuilderState, 'name'> & {
    name: FlagName.Data.FlagNames
  }

  // prettier-ignore
  export type CreateParameterFromParameter<
    State extends Base,
    S     extends MinimumParameterBuilderState,
  > = {
    // Schema: S['type'] extends Type.String ? z.ZodString : never // todo Configuration['type']
    // NameParsed: FlagName.Parse<S['name']['canonical'], { usedNames: GetUsedNames<State>; reservedNames: ReservedParameterNames }>
    // NameUnion: FlagName.Data.GetNamesFromParseResult<FlagName.Parse<S['name'], { usedNames: GetUsedNames<State>; reservedNames: ReservedParameterNames }>>
  }
