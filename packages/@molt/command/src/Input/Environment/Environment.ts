...
export const parse = (environment: RawInputs, specs: Spec[]): Index<ArgumentReport> => {
  const envars = Object.entries(environment)
    ...
    .filter((envar): envar is { name: { raw: string; camel: string }; value: string } => envar !== null)
  const reportsIndex: Index<ArgumentReport> = {}
  const specsPool = specs.filter((spec) => spec.environment !== null)
  const specsSeen: Index<true> = {}
  ...
  for (const envar of envars) {
    for (const spec of specsPool) {
      if (specsSeen[spec.name.canonical]) continue
      ...
      if (isNamespaced) {
        ...
        const namespaceAndNameHit = spec
          .environment!.namespaces.flatMap((namespace) =>
            specParameterNames.map((name) => ({ namespace, name }))
          )
