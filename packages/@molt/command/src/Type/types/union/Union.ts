...
export interface Union<
  $Members extends readonly Union.Member[] = Union.Member[],
...
export const union = <$Members extends readonly Union.Member[]>({
  ...
  return {
    ...
    help: (settings) => {
      const hasAtLeastOneMemberDescription =
        members.filter((_) => _.description !== null).length > 0

      const isExpandedMode =
        (hasAtLeastOneMemberDescription &&
          settings?.helpRendering?.union?.mode ===
            `expandOnParameterDescription`) || // eslint-disable-line
        settings?.helpRendering?.union?.mode === 'expandAlways' // eslint-disable-line
      const unionMemberIcon = Term.colors.accent(`◒`)
      const isOneOrMoreMembersWithDescription = members.some(
        (_) => _.description !== null,
      )
      const isExpandedModeViaForceSetting =
        isExpandedMode && !isOneOrMoreMembersWithDescription
      if (isExpandedMode) {
        ...
        return Tex.block((__) =>
          __.block(
            Term.colors.dim(
              Text.chars.borders.leftTop +
                Text.chars.borders.horizontal +
                `union`,
            ),
          )
            .block(
              {
                padding: { bottom: 1 },
                border: {
                  left: `${Term.colors.dim(Text.chars.borders.vertical)} `,
                },
              },
              description ?? null,
            )
            ...
            .block(
              Term.colors.dim(
                Text.chars.borders.leftBottom + Text.chars.borders.horizontal,
              ),
            ),
        ) as Tex.Block
      } else {
        const membersRendered = members
          .map((m) => m.displayExpanded())
          .join(` | `)
        return Tex.block(($) =>
          $.block(membersRendered).block(description ?? null),
        ) as Tex.Block
      }
    },
    validate: (value) => {
      const result = members.find(
        (member) => member.validate(value)._tag === `Right`,
      )
      return result
        ? Either.right(value)
        : Either.left({
            value,
            errors: [`Value does not fit any member of the union.`],
          })
    },
    prompt: (params) =>
