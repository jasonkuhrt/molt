...
export const inputForParameter = (params: Params<Pam.Parameter>) => {
  ...
  if (parameter.type._tag === `TypeUnion`) {
    return Inputs.union(params as Params<Pam.Parameter<Type.Union>>)
  }

  if (parameter.type._tag === `TypeBoolean`) {
    return Inputs.boolean(params as Params<Pam.Parameter<Type.Scalar.Boolean>>)
  }

  if (parameter.type._tag === `TypeString`) {
    return Inputs.string(params as Params<Pam.Parameter<Type.Scalar.String>>)
  }

  if (parameter.type._tag === `TypeNumber`) {
    return Inputs.number(params as Params<Pam.Parameter<Type.Scalar.Number>>)
  }

  if (parameter.type._tag === `TypeEnum`) {
    return Inputs.enumeration(params as Params<Pam.Parameter<Type.Scalar.Enumeration>>)
  }
