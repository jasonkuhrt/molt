...
type VDefaultNarrower = BuilderKit.State.Values.Atom<number, 1>
type VValueSet = BuilderKit.State.Values.Atom<number> & { value: 1 }
...
type VA2Set = BuilderKit.State.Values.Atom<number, number> & { value: 1 }
...
type S<A> = { data: { a: A }; resolve: null; name: string }
type S1 = { data: { a: VA1 }; resolve: null; name: string }
type S2 = { data: { a: VA2 }; resolve: null; name: string }
type S2Set = { data: { a: VA2Set }; resolve: null; name: string }
...
type SB = { data: { a: VValueSet }; resolve: null; name: string }
...
test('State.Property.Value.*', () => {
  ...
  expectTypeOf<BuilderKit.State.Property.Get<S1,'a'>>().toEqualTypeOf<VA1>()
  //---
  expectTypeOf<BuilderKit.State.Property.Value.Get<S1,'a'>>().toEqualTypeOf<number|BuilderKit.State.Values.Unset>()
  //---
  ...
  //---
  expectTypeOf<BuilderKit.State.Property.Value.GetAsSet<S1, 'a'>>().toEqualTypeOf<number>()
  expectTypeOf<BuilderKit.State.Property.Value.GetAsSet<{data:{ a: VValueSet };resolve:null; name:string }, 'a'>>().toEqualTypeOf<1>()
  // @ts-expect-error test
  expectTypeOf<BuilderKit.State.Property.Value.GetAsSet<{ a: VValueSet }, 'a'>>().toEqualTypeOf<2 | BuilderKit.State.Values.Unset>()
  //---
  expectTypeOf<BuilderKit.State.Property.Value.IsSet<{data:{ a: VValueSet };resolve:null; name:string }, 'a'>>().toEqualTypeOf<true>()
  expectTypeOf<BuilderKit.State.Property.Value.IsSet<S1, 'a'>>().toEqualTypeOf<false>()
  //---
  expectTypeOf<BuilderKit.State.Property.Value.IsUnset<{data:{ a: VValueSet };resolve:null; name:string }, 'a'>>().toEqualTypeOf<false>()
  expectTypeOf<BuilderKit.State.Property.Value.IsUnset<S1, 'a'>>().toEqualTypeOf<true>()
  //---
  expectTypeOf<BuilderKit.State.Property.Value.Set<S1, 'a', 1>>().toMatchTypeOf<{data:{ a: VValueSet };resolve:null; name:string }>()
  //---
  expectTypeOf<BuilderKit.State.Property.Value.SetAll<S1, { a:1 }>>().toMatchTypeOf<{data:{ a: VValueSet };resolve:null; name:string }>()
  expectTypeOf<BuilderKit.State.Property.Value.SetAll<{data:{ a: VA1, b: VValueSet, c: VA1 };resolve:null; name:string }, { a:1 }>>().toMatchTypeOf<{data:{ a: VValueSet /* < */, b: VValueSet, c: VA1 };resolve:null; name:string }>()
  //---
  expectTypeOf<BuilderKit.State.Setup<S1, { x: 0 }>>().toEqualTypeOf<BuilderKit.SetupHost<{data:{a:VA1};resolve:null; name:string },{x:0}>>()
  //---
  expectTypeOf<BuilderKit.State.Get<BuilderKit.WithMinState<B1Fn, SA, { a:1 }>>>().toMatchTypeOf<SB>()
})
...
test('State.Initial', () => {
  ...
  expectTypeOf<BuilderKit.State.RuntimeData<{data:{ a: VValueSet };resolve:null; name:string }>>().toEqualTypeOf<{ readonly a:1 }>()
  // @ts-expect-error test
  expectTypeOf<BuilderKit.State.RuntimeData<{ a: VValueSet }>>().toEqualTypeOf<{ a: 2 | BuilderKit.State.Values.Unset }>()
  // @ts-expect-error test
...
// prettier-ignore
test('WithMinState', () => {
  expectTypeOf<BuilderKit.WithMinState<B1Fn, SA, { a:1 }>>().toMatchTypeOf<B1<SB>>()
})
...
test('toRuntime', () => {
  ...
  // with default equal to type
  expectTypeOf<BuilderKit.State.ToRuntime<S2>>().toEqualTypeOf<{ resolve:()=>null; name:string; data: { readonly a:number }}>() // prettier-ignore
  // with default narrower than type
  expectTypeOf<BuilderKit.State.ToRuntime<S<VDefaultNarrower>>>().toEqualTypeOf<{ resolve:()=>null; name:string; data: { readonly a:1 }}>() // prettier-ignore
  // with default AND value (narrower than type) set
  expectTypeOf<BuilderKit.State.ToRuntime<S2Set>>().toEqualTypeOf<{ resolve:()=>null; name:string; data: { readonly a:1 }}>() // prettier-ignore
})
